...
def load_cashflow_and_pnl():
    """Load cashflow and pnl."""
    con = sqlite3.connect(DB_PATH)
    cashflow_df = pd.read_sql("SELECT * FROM cashflow ORDER BY company_id, year", con)
    pnl_df = pd.read_sql(
        "SELECT company_id, year, net_profit FROM profitandloss ORDER BY company_id, year", con
    )
    con.close()
...
def merge_into_cashflow_intelligence(latest_alloc_df):
    ...
    check = ci_df.merge(latest_alloc_df[["company_id", "pattern_label"]], on="company_id", how="left")
    mismatches = check[check["capital_allocation_label"].fillna("") != check["pattern_label"].fillna("")]
    return mismatches
