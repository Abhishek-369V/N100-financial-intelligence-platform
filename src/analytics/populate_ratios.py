...
from cashflow_kpis import fcf_conversion_rate, free_cash_flow
from ratios import (
    asset_turnover,
    debt_to_equity,
    high_leverage_flag,
...
def populate_all_companies(pnl, bs, cf):
    ...
    for company_id in companies:
        ...
        for _, pnl_row in company_pnl.iterrows():
            ...
            opm, _opm_mismatch = operating_profit_margin(
            ...
            return_on_assets(pnl_row["net_profit"], bs_row["total_assets"])

            de = debt_to_equity(bs_row["borrowings"], bs_row["equity_capital"], bs_row["reserves"])
            icr = interest_coverage_ratio(
                pnl_row["operating_profit"], pnl_row["other_income"], pnl_row["interest"]
            )
            ...
            fcf_conversion_rate(fcf, pnl_row["operating_profit"])

            cfo_pat_ratio = None
            if pnl_row["net_profit"] != 0:
                cfo_pat_ratio = round(cf_row["operating_activity"] / pnl_row["net_profit"], 2)

            composite_quality_score(roe, icr, cfo_pat_ratio)
            ...
            results.append(
                {
                    ...
                    "high_leverage_flag": high_leverage_flag(
                        de, None
                    ),  # sector arg added in Day 13 carve-out
...
def manual_spot_check(df, sample_companies=3):
    ...
    for cid in sample_ids:
        ...
        print("     from balancesheet/profitandloss, recompute by hand, compare within 0.1%")
