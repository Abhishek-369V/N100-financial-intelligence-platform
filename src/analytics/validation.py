...
def load_data():
    ...
    ratios = pd.read_sql("SELECT company_id, year, free_cash_flow_cr FROM financial_ratios", db_engine)
...
def apply_valuation_flag(row):
    """Apply valuation flag for the given row."""
...
def generate_valuation_summary():
    ...
    latest_mc = latest_year_rows(market_cap, ["market_cap_crore", "pe_ratio", "pb_ratio", "ev_ebitda"])
    ...
    summary = summary.merge(
        pe_5yr.rename("pe_5yr_median"), left_on="company_id", right_index=True, how="left"
    )
