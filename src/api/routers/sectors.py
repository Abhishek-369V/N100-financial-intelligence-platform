...
def list_sectors(conn=Depends(get_db_connection)):
    sector_names = [r["broad_sector"] for r in conn.execute(
    ...
    return {"count": len(results), "note": "10 sectors in this dataset, not 11 — see module docstring", "sectors": results}


@router.get("/sectors/{sector_name}/companies")
def get_sector_companies(sector_name: str, conn=Depends(get_db_connection)):
    known_sectors = {r["broad_sector"] for r in conn.execute("SELECT DISTINCT broad_sector FROM sectors").fetchall()}
