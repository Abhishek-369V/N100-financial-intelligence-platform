...
def is_monotonic_increasing(series):
    """Is monotonic increasing for the given series."""
    return all(b >= a for a, b in itertools.pairwise(series))


def is_monotonic_decreasing(series):
    """Is monotonic decreasing for the given series."""
    return all(b <= a for a, b in itertools.pairwise(series))
...
def evaluate_company(company_id, r, p):
    ...
    if latest is not None:
        ...
        div_yield_ok = (
            latest["dividend_payout_ratio_pct"] is not None and latest["dividend_payout_ratio_pct"] > 0
        )
        ...
        if (
            latest["revenue_cagr_5yr"] is not None
            and latest["pat_cagr_5yr"] is not None
    ...
    if latest is not None:
        ...
        if len(p) >= 2:
            ...
            streak = consecutive_from_end(
                [b - a for a, b in itertools.pairwise(sales_series)], lambda diff: diff < 0
            )
            if streak >= 2:
                results.append(
                    _row(
...
def _row(company_id, type_, rule_id, confidence, text):
    """Row for the given company_id, type_, rule_id, confidence, text."""
    return {
    ...
    for company_id in all_company_ids:
        ...
        if not has_con:
            ...
            if is_financial and len(cr) and cr.iloc[-1]["operating_profit_margin_pct"] is not None:
                ...
                not is_financial
                and len(cr)
                and cr.iloc[-1]["debt_to_equity"] is not None
                and cr.iloc[-1]["debt_to_equity"] > 0
            ):
...
def generate_pros_cons():
    ...
    for company_id in companies["company_id"]:
        ...
        rows = [
            row for row in evaluate_company(company_id, r, p) if row["confidence_pct"] > CONFIDENCE_THRESHOLD
        ]
    ...
    add_fallback_coverage(
        results_by_company, companies["company_id"].tolist(), ratios, pnl, financial_company_ids
    )
