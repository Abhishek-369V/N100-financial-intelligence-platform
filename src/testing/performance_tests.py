...
def load_test_screener(n_requests=10):
    """Load test screener for the given n_requests."""
    results = []
    lock = threading.Lock()
...
def time_company_profile_data_load(tickers):
    """Time company profile data load for the given tickers."""
    sys.path.insert(0, str(BASE_DIR / "src" / "dashboard"))
    from utils.db import get_companies, get_pl, get_ratios  # type: ignore
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"All 10 completed in {overall_duration:.2f}s (target: <10s) — {'PASS' if overall_duration < 10 else 'FAIL'}"
        )
