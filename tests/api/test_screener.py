...
def test_screener_max_de_does_not_exempt_non_financials(client):
    ...
    response = client.get("/api/v1/screener?max_de=1.0")
    non_financial_ids = {
        c["company_id"] for c in response.json()["companies"] if c["broad_sector"] != "Financials"
    }
    all_response = client.get("/api/v1/screener")
    all_non_financial_ids = {
        c["company_id"] for c in all_response.json()["companies"] if c["broad_sector"] != "Financials"
    }
