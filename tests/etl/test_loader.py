...
from loader import CORE_FILES, SUPPORTING_FILES, load_core_file, load_supporting_file
...
def test_cashflow_columns():
    df = load_core_file(CORE_FILES["cashflow"])
    assert {"company_id", "year", "operating_activity", "investing_activity", "financing_activity"}.issubset(
        df.columns
    )
